    ...
    def add_routes(self):
        ...
        def get_versions():
            ...
            if result.stdout:
                ...
                pattern = re.compile(r"refs/heads/openshift-4\.(\d+)$")
                versions = []
                ...
                versions = [version.replace('openshift', 'okd') for version in versions]
                sorted_ocp_versions = sorted(
                    versions,
                    key=lambda x: tuple(map(int, x.replace("okd-", "").split("."))),
                    reverse=True
                )
